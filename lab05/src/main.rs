...
fn ex1() {
    ...
    let mut cnt = 0;
    let mut array: [(&str, &str, u32); 100] = [("", "", 0); 100];
    for l in s.lines() {
        let mut it = l.split(",");
        let nume = it.next();
        let numar = it.next();
        let varsta = it.next();
        if let (Some(nume), Some(numar), Some(varsta)) = (nume, numar, varsta) {
            let mut nr = 0;
            for c in varsta.chars() {
                nr = nr * 10 + (c as u8 - b'0') as u32;
            }
            array[cnt] = (nume, numar, nr);
            cnt += 1;
        }
    }
    array[..cnt].sort_by_key(|t| t.2);
    println!("The oldest student is {}, {} years old.", array[cnt - 1].0, array[cnt - 1].2);
    println!("The youngest student is {}, {} years old.", array[0].0, array[0].2);
}
fn new_canvas() -> [[char; 100]; 55] {
    [[' '; 100]; 55]
}
fn set_pixels(c: &mut [[char; 100]; 55], p: &[(usize, usize, u8)]) {
    for e in p {
        c[e.0][e.1] = e.2 as char;
    }
}
